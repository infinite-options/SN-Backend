...
def kitchenExists(kitchen_id):
    ...
    kitchen = execute(
        """SELECT * FROM sn_kitchens WHERE kitchen_id = \'""" + kitchen_id + """\';""", 'get', conn, skipSerialization=True)    
    ...
    #     }
    # )
    return True if len(kitchen['result']) > 0 else False
    #return not kitchen.get('Items') == []

def couponExists(coupon_id):
    conn = connect()
    coupon = execute(
        """SELECT * FROM sn_coupons WHERE coupon_id = \'""" + coupon_id + """\';""", 'get', conn, skipSerialization=True)
    ...
    #     }
    # )
    return True if len(coupon['result']) > 0 else False
    #return not coupon.get('Items') == []

class Test(Resource):
    def get(self):
        return couponExists('SNMay2020')
# ===========================================================
...
class Kitchens(Resource):
    def get(self):
        ...
        try:
            conn = connect()
            kitchens = execute(
                """SELECT kitchen_name, kitchen_id, close_time, description, open_time, isOpen, accepting_hours, is_accepting_24hr, delivery_hours, zipcode, available_zipcode
                    FROM sn_kitchens;""", 'get', conn, skipSerialization=True)
            ...
            for kitchen in kitchens['result']:
                kitchen['open_time'] = formateTime(kitchen['open_time'])
                kitchen['close_time'] = formateTime(kitchen['close_time'])

                if kitchen['isOpen'] == True:
...
class Saved_Nofitication_Group(Resource): 
    ...
    def post(self):
        ...
        if 'customers' not in data:
            raise BadRequest('Missing customers.')
        print(data)
        #print(f"group_name:{data['group_name']}, customers:{data['customers']}")
        return 200
